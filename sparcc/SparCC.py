...
from sparcc.core_methods import to_fractions
from sparcc.compositional_methods import run_clr,variation_mat
...
def preprocess_zero_values(C: np.ndarray) -> np.ndarray:
    '''Zero-value exclusion: Remove rows/columns that have all zeros in the pair.'''
    # Identify rows and columns where all values are zero
    zero_rows = np.all(C == 0, axis=1)
    zero_cols = np.all(C == 0, axis=0)
    
    # Remove the rows and columns that are all zeros
    C_cleaned = np.delete(C, np.where(zero_rows)[0], axis=0)
    C_cleaned = np.delete(C_cleaned, np.where(zero_cols)[0], axis=1)
...
def new_excluded_pair(C: Any, previously_excluded: List = [], th: float = 0.1):
    ...
    weren't previously excluded, after handling zero-value OTU pairs.
    '''
    # Preprocess zero values
    C = preprocess_zero_values(C)
